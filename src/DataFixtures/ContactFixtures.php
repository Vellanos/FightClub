...
class ContactFixtures extends AbstractFixtures implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            $contact->setTitle($titles[$this->faker->numberBetween(1, 19)]);
            $contact->setContent($comments[$this->faker->numberBetween(1, 19)]);
            $contact->setRecurring($this->faker->numberBetween(1, 5));
            $contact->setNote($this->faker->numberBetween(1, 5));
            $contact->setUser($this->getReference('user_' . $this->faker->numberBetween(0, 9)));
